...
esp32 BLE compile test via AUTO_LOAD, but the RTL-specific side — automation.h's
action templates and restart_scan_duration() — compiles on no CI board (the
rtl87xx base board is RTL8710B, which has no BLE and cannot build the tracker),
and validate fixtures never run to_code. The generated main is therefore the only
