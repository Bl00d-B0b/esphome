"""
BK72xx BLE Tracker — ESPHome BLE 5.x scanner for the BLE-capable LibreTiny Beken
chips (beken-72xx family): BK7231N and BK7238.
...
Requires a BLE-5.x-capable Beken chip — BK7231N or BK7238. BK7231T (BLE 4.2, no
5.x scan slot), BK7231Q (no BLE) and BK7251 are NOT supported (rejected at
compile time in to_code()).
...
Requires:
  - LibreTiny platform (beken-72xx family, BK7231N)
...
# Requires the bk72xx platform. The component works on the BLE-5.x Beken families
# (BK7231N and BK7238); to_code() rejects the others (BK7231T, BK7231Q, BK7251 —
# BLE 4.2 / none) with a clear error instead of a cryptic link failure against the
# BK BLE 5.x SDK.
DEPENDENCIES = ["bk72xx"]
CODEOWNERS = ["@Bl00d-B0b"]

# BLE 5.x Beken chip families supported by the BDK BLE stack (libble_<chip>.a).
BLE5_BK_FAMILIES = [FAMILY_BK7231N, FAMILY_BK7238]


def _require_ble5_family():
    """Reject non-BLE-5.x Beken chips at compile time.

    Enforced here in to_code() rather than FINAL_VALIDATE_SCHEMA so a plain
    `esphome config` still validates on any bk72xx board, while an actual build on
    a BK7231T/BK7231Q/BK7251 fails early with a clear message instead of a cryptic
    link error against the BK BLE 5.x SDK.
    """
    family = libretiny.get_libretiny_family()
    if family not in BLE5_BK_FAMILIES:
        raise cv.Invalid(
            f"bk72xx_ble_tracker requires a BLE 5.x Beken chip "
            f"({' or '.join(BLE5_BK_FAMILIES)}); the selected board is {family}, "
            f"which has no usable BLE 5.x scan (BK7231T/BK7231Q/BK7251 are BLE 4.2 or none)."
        )
...
async def to_code(config):
    # Compile-time gate: only BK7231N / BK7238 link against the BLE 5.x SDK.
    _require_ble5_family()
