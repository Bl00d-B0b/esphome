...
// ServiceData, ESPBLEiBeacon, ESPBTDevice, ESPBTDeviceListener) on the BK72xx
// so that BLE sensor components written for ESP32 compile without changes.
//
// Guarded by #ifdef USE_LIBRETINY so it is a no-op on every non-LibreTiny build.
...
class ESPBTUUID {
 public:
  ...
  /// Deprecated alias mirroring esp32_ble's ESPBTUUID, so existing esp32 BLE lambdas
  /// that call uuid.to_string() compile unchanged on this backend.
  ESPDEPRECATED("Use to_str() instead. Removed in 2026.8.0", "2026.2.0")
  std::string to_string() const { return this->to_str(); }  // NOLINT
...
class ESPBTDevice {
 public:
  /// Populate from a raw BK72xx scan result.
  ...
  /// esp32_ble_tracker::ESPBTDevice::resolve_irk. Implemented in the .cpp via mbedtls
  /// AES-128 when <mbedtls/aes.h> is available; on a platform without it this returns
  /// false (RPA/IRK matching disabled) rather than failing the build.
...
// so that sensor components compiled with the BK72xx backend can include this
// header and use esp32_ble_tracker:: qualified names without modification.
// ---------------------------------------------------------------------------
namespace esphome::esp32_ble_tracker {
using adv_data_t = esphome::bk72xx_ble_tracker::adv_data_t;
using ESPBTUUID = esphome::bk72xx_ble_tracker::ESPBTUUID;
using ServiceData = esphome::bk72xx_ble_tracker::ServiceData;
using ESPBLEiBeacon = esphome::bk72xx_ble_tracker::ESPBLEiBeacon;
using ESPBTDevice = esphome::bk72xx_ble_tracker::ESPBTDevice;
using ESPBTDeviceListener = esphome::bk72xx_ble_tracker::ESPBTDeviceListener;
}  // namespace esphome::esp32_ble_tracker
