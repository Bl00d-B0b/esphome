...
// builder (CFG_SUPPORT_BLE=1, CFG_BLE_VERSION=BLE_VERSION_5_x in the chip's
// sys_config.h; prebuilt libble_<chip>.a). This component only calls into it —
...
// Matches ESP32's MAX_BLE_QUEUE_SIZE without PSRAM (the BK72xx have no PSRAM).
...
class BK72xxBLETracker : public Component {
 ...
 public:
  ...
  bool proxy_scan_active() override { return false; }  // BK72xx scan passive-only
 ...
 protected:
  ...
  bool restart_pending_{false};  // non-continuous: a failed watchdog restart needs retrying from loop()
