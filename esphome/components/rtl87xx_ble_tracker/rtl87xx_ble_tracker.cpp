...
static void rtl_ble_adv_cb(const uint8_t bd_addr[6], uint8_t addr_type, uint8_t adv_type, int8_t rssi,
...
void RTL87xxBLETracker::on_raw_report(const uint8_t mac[6], uint8_t addr_type, uint8_t adv_type, int8_t rssi,
                                      const uint8_t *data, uint8_t len) {
...
void RTL87xxBLETracker::get_adapter_mac(uint8_t out[MAC_ADDRESS_SIZE]) { this->parent_->get_mac(out); }
