// ln882h_ble_device.cpp
//
// Implementation of the esp32_ble_tracker compatibility types for LN882H.
...
// LibreTiny chip/framework. Guard the include so the component still builds where the
// header is absent; resolve_irk() then degrades to "no match" (see its body).
#if __has_include(<mbedtls/aes.h>)
#include <mbedtls/aes.h>
#define LN882H_BLE_HAS_MBEDTLS_AES
#endif
...
const char *ESPBTDevice::address_str_to(char *buf) const {
  // LN882H stores MAC bytes as [0]=LSB … [5]=MSB; display MSB-first.
...
#ifndef LN882H_BLE_HAS_MBEDTLS_AES
