...
class LN882HBLETracker : public Component, public bluetooth_proxy::BleProxyHub {
 ...
 protected:
  ...
  // True once any advertisement/scan-response has ever been received. Gates the
  // watchdog reboot: a scanner that has never seen a single frame is most likely
  // in a BLE-quiet environment, not wedged, so it must not reboot-loop.
