...
static constexpr uint8_t GAPM_SCAN_PROP_PHY_CODED_BIT = 1 << 2;
static constexpr uint8_t GAPM_SCAN_PROP_ACTIVE_CODED_BIT = 1 << 3;
...
// proportion.  The reboot guard escalates after 20 consecutive restarts with no
// results in between (~1 min at the default interval) before rebooting the device.
...
void LN882HBLETracker::loop() {
  ...
  // Drain the advertisement queue populated by rw_task.
  // Swap under critical section so rw_task is blocked for the minimum time.
  if (!this->adv_queue_.empty()) {
    ...
    // Log dropped advertisements — mirrors ESP32's "Dropped %u BLE events due to buffer overflow".
    // Only reachable when events were processed; no point checking if the queue was empty.
    // Read and reset under critical section to avoid losing a concurrent rw_task increment.
    taskENTER_CRITICAL();
  ...
  if (this->scan_continuous_) {
    if (this->scan_running_) {
      ...
      if (this->last_scan_result_time_ != 0 && millis() - this->last_scan_result_time_ > watchdog_ms) {
        ...
        // The reboot guard handles true hardware failures.
...
void LN882HBLETracker::dump_config() {
  ...
  ESP_LOGCONFIG(TAG, "  Scan mode:       %s", this->scan_active_ ? "active" : "passive");
...
void LN882HBLETracker::on_scan_rsp(const uint8_t *mac, int rssi, uint8_t addr_type, const uint8_t *data,
                                   uint16_t data_len) {
  // Scan-response data goes to the Bluetooth proxy only (HA merges per address);
  // local listeners/triggers receive each advertisement exactly once via
  // on_scan_result. Counts as scan activity for the watchdog.
...
void LN882HBLETracker::on_scan_result(const uint8_t *mac, int rssi, uint8_t addr_type, const uint8_t *data,
  ...
  // Raw callback for bluetooth proxy — runs here in rw_task context.
  if (this->scan_result_callback_)
    this->scan_result_callback_(mac, rssi, addr_type, data, data_len);
  ...
  // in loop() on the main task, so log lines never carry the rw_task tag.
...
void LN882HBLETracker::start_scan_() {
  ...
  p.prop = GAPM_SCAN_PROP_PHY_1M_BIT | GAPM_SCAN_PROP_PHY_CODED_BIT;
  if (this->scan_active_)
    p.prop |= GAPM_SCAN_PROP_ACTIVE_1M_BIT | GAPM_SCAN_PROP_ACTIVE_CODED_BIT;
