...
#ifdef USE_ESP32_BLE_DEVICE
// Adapt an ESP-IDF scan result into the neutral device type. BLEScanResult's
// bda is most-significant octet first; the neutral from_scan_result takes the
// BLE controller (LSB-first) order, so reverse — address_uint64()/address_str()
// then produce exactly the historical esp32 values.
static void espbt_device_from_scan_rst(ESPBTDevice &device, const BLEScanResult &scan_result) {
  uint8_t mac_lsb_first[6];
  for (uint8_t i = 0; i < 6; i++)
    mac_lsb_first[i] = scan_result.bda[5 - i];
  device.from_scan_result(mac_lsb_first, scan_result.rssi, scan_result.ble_addr_type, scan_result.ble_adv,
...
#ifdef USE_ESP32_BLE_DEVICE
    ESPBTDevice device;
    espbt_device_from_scan_rst(device, scan_result);
