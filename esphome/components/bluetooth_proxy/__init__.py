...
CODEOWNERS = ["@jesserockz", "@bdraco", "@Bl00d-B0b"]
...
if CORE.is_esp32:
    # ---------------------------------------------------------------------------
    # ESP32: full proxy — advertisement forwarding + GATT active connections.
    # ---------------------------------------------------------------------------
    from esphome.components import esp32_ble, esp32_ble_client, esp32_ble_tracker
    from esphome.components.esp32 import add_idf_sdkconfig_option
    from esphome.components.esp32_ble import BTLoggers
    from esphome.const import CONF_ACTIVE
    ...
    async def to_code(config):
        ...
        for connection_conf in config.get(CONF_CONNECTIONS, []):
            ...
            await esp32_ble_tracker.register_raw_client(
                connection_var, connection_conf
            )
    ...
    # ---------------------------------------------------------------------------
    # LibreTiny (bk72xx / ln882h / …): advertisement proxy only. Attaches to a
    # tracker hub that implements BleProxyHub; GATT active connections are ESP32-only.
    # ---------------------------------------------------------------------------
    DEPENDENCIES = ["api"]

    BluetoothProxy = bluetooth_proxy_ns.class_("BluetoothProxy", cg.Component)
    # Interface implemented by tracker hubs; cv.use_id finds the configured tracker.
