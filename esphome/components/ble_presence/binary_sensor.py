...
async def to_code(config):
    ...
    if service_uuid := config.get(CONF_SERVICE_UUID):
        if len(service_uuid) == len(ble_device_base.bt_uuid16_format):
            cg.add(var.set_service_uuid16(ble_device_base.as_hex(service_uuid)))
        elif len(service_uuid) == len(ble_device_base.bt_uuid32_format):
            cg.add(var.set_service_uuid32(ble_device_base.as_hex(service_uuid)))
        elif len(service_uuid) == len(ble_device_base.bt_uuid128_format):
            uuid128 = ble_device_base.as_reversed_hex_array(service_uuid)
